...
AI_FEATURES_AVAILABLE = bool(ANTHROPIC_API_KEY or OPENAI_API_KEY or GEMINI_API_KEY or GROQ_API_KEY)
...
HOW_IT_WORKS_GUIDE = {
    ...
    "Communication & Admin": {
        ...
        "Chat": "Global, Supervisor-only, and private messaging, plus an AI Assistant room "
            ...
            "data, and general knowledge questions too.",
...
# ---- CHAT ROOM ----
    ...
    if room == "global":
        ...
        if not _focused_mode:
            # The focused-mode header above already shows this same
            # framing more prominently — showing it again here would
            # be redundant, so it's only rendered when reaching this
            # room the normal way (via the room switcher).
            st.markdown(
                '<div class="chat-room-header"><div class="chat-room-icon">'
                '<i class="fas fa-robot"></i></div><div>'
                '<div class="chat-room-title">AI Assistant</div>'
                '<div class="chat-room-sub">Answers questions about this app, grounded in its current data — '
                'and general knowledge questions too</div>'
                '</div></div>', unsafe_allow_html=True)
        ...
        _ai_question = st.text_input("Ask anything — about this app, or general knowledge...", key="ai_assistant_input")
