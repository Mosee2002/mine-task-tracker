...
# AI-assisted features share this one config. Supports four
...
# and Groq added specifically because they offer genuine free tiers
# (no payment method required), unlike Anthropic/OpenAI which are
# both usage-billed with no free tier — this matters for a site that
# wants AI features without a card on file.
...
GEMINI_MODEL = _secret_get("GEMINI_MODEL", "gemini-2.5-flash")
# The model name was never actually the problem — confirmed directly
# via a real deployment's own query against
# https://generativelanguage.googleapis.com/v1beta/models?key=...,
# which listed gemini-2.5-flash first with generateContent explicitly
# supported. The real bug was the auth method (see generate_smart_text
# below — now uses ?key= query param, not the x-goog-api-key header,
# since only the former was confirmed to actually work for that key).
# If this model ever stops working for a given account, the same
# models-list URL (with that account's own key) is the reliable way
# to find a working replacement — override via GEMINI_MODEL in
# secrets.toml, no code change needed.
GROQ_API_KEY = _secret_get("GROQ_API_KEY", "")
# Groq's API is genuinely OpenAI-compatible (same endpoint shape,
# same request/response format, confirmed directly against Groq's own
# docs and GitHub repo, not assumed from the marketing claim alone) —
# this is why its branch in generate_smart_text() below can reuse the
# OpenAI request logic almost as-is, just with a different base URL,
# key, and model. Also has a genuine free tier.
GROQ_MODEL = _secret_get("GROQ_MODEL", "llama-3.3-70b-versatile")
# Model naming here has moved fast (3.1 -> 3.3 -> Llama 4 variants
# across different current Groq sources) — genuinely uncertain which
# exact name is most current as of this writing, so this picks the
# one most consistently shown in Groq's own stable docs pages rather
# than a specific numbered example that might already be superseded.
# Override via GROQ_MODEL in secrets.toml if this 404s — Groq's
# console (console.groq.com) lists your account's available models
# directly, the same kind of check that resolved the Gemini issue.
OPENROUTER_API_KEY = _secret_get("OPENROUTER_API_KEY", "")
# Also genuinely OpenAI-compatible (confirmed directly against
# OpenRouter's own docs, not assumed) — but with one real difference
# from Groq/OpenAI: the model field needs a provider prefix, e.g.
# "openai/gpt-4o-mini" or "google/gemini-2.0-flash-001", not a bare
# model name. Positioned as the fallback AFTER Groq specifically —
# the practical version of "if Groq can't handle it, this steps in":
# genuinely detecting task difficulty before trying would need a
# separate AI call just to judge it, adding cost and latency to every
# question, so this activates on an actual Groq failure (error,
# rate limit, timeout), not a difficulty guess made in advance.
OPENROUTER_MODEL = _secret_get("OPENROUTER_MODEL", "openai/gpt-4o-mini")
AI_FEATURES_AVAILABLE = bool(ANTHROPIC_API_KEY or OPENAI_API_KEY or GEMINI_API_KEY
                            or GROQ_API_KEY or OPENROUTER_API_KEY)
...
/* The active nav button is always disabled=True (see the sidebar nav
   code) — without this, it fell back to Streamlit's own default
   disabled-button appearance, which isn't paired with this app's
   theme variables the way the rest of the sidebar is, and reads
   poorly in at least one of the two themes. */
...
/* Floating AI Assistant button — a real st.button() inside a
   st.container(key=...), styled here to look and behave like a
   floating action button. Targets the button as a DESCENDANT of the
   keyed container ([class*="st-key-..."] button), not the container
   itself directly — Streamlit has a documented quirk (streamlit/
   streamlit#10674) where the generated .st-key-<name> class doesn't
   always land on the container's own outermost element, so this is
   the more robust selector, not a shortcut.

   Bottom-CENTER, not bottom-right — horizontally centered via
   left:50% + a counter-transform, the standard way to center a
   fixed-position element whose own width varies (a %-based left
   alone would only center the element's top-left corner, not the
   element itself). */
[class*="st-key-floating-ai-toggle"] button {
    position: fixed !important;
    bottom: 20px;
    left: 50%;
    transform: translateX(-50%);
    z-index: 999997;  /* below the sticky header (999999) and logo bar (999998), so neither ever hides it, but it never covers them either */
    width: 60px;
    height: 60px;
...
/* ---------- Alerts: explicit, theme-adaptive background+text pairing,
   not just a text-color patch. The previous version only forced dark
   text (its own comment admitted this was specifically to fix
   DARK-mode washout) while leaving the background entirely to
   Streamlit's own native theme handling — which isn't guaranteed to
   track this app's own light/dark toggle. That one-sided fix meant
   whichever theme it WASN'T written for could still end up with
   poor contrast. Setting both sides together removes the guesswork
   regardless of what Streamlit's native alert background would
   otherwise have been. ---------- */
[data-testid="stAlert"] {
    border-radius: 10px;
...
# Keys that are internal bookkeeping, never useful to show a person —
# every log entry carries a "version" for optimistic-concurrency
# purposes, but nobody reading an activity feed cares that a task is
# now on version 3.
_LOG_DETAIL_HIDDEN_KEYS = {"version", "id", "task_id"}


def humanize_log_details(details):
    """Turns a raw audit-log details dict into a short, readable phrase
    instead of dumping its JSON verbatim — {"assigned_to": "Moses
    Anaba", "version": 3} becomes "Assigned to: Moses Anaba", not
    literal curly braces and quote marks on screen.

    A lone "comment" key is returned as just the comment text itself
    — the surrounding log line already says "X commented on Y", so
    prefixing the text again with "Comment:" would be redundant.

    Every other key is converted from snake_case to Title Case,
    paired with its value, and joined with the same middle-dot
    separator already used elsewhere in this app's log rendering.
    Keys in _LOG_DETAIL_HIDDEN_KEYS are dropped entirely (internal
    bookkeeping, not informative to a reader), as are keys whose
    value is None/null/empty — "Completed At: None" tells a reader
    nothing they didn't already know from the entry not being marked
    complete.
    """
    if not isinstance(details, dict) or not details:
        return ""
    if set(details.keys()) == {"comment"}:
        return str(details["comment"])
    parts = []
    for key, value in details.items():
        if key in _LOG_DETAIL_HIDDEN_KEYS:
            continue
...
def render_log_entries(entries, actor_key="user_name", action_key="action",
    ...
    for e in entries:
        ...
        details_html = ""
        if details:
            details_str = details if isinstance(details, str) else humanize_log_details(details)
            if details_str and len(details_str) > 300:
...
def render_subheading(text, level=4):
    """A translated sub-heading WITHIN a section — deliberately keeps
    markdown's smaller visual weight rather than promoting these to
    the big centered treatment render_section_header() gives. These
    are things like "Create New Purchase Order" sitting inside
    Inventory; making them as prominent as the section title itself
    would flatten the page's hierarchy, not improve it. The only real
    gap here was translation, so that's the only thing this changes.
    """
    st.markdown(f"{'#' * level} {auto_t(text)}")


def render_section_header(title_with_emoji, subtitle=None):
    """A polished, centered replacement for a plain st.subheader() —
    same visual pattern already proven for the Chat section header.
    Accepts the exact same "🏭 Production Tracking"-style string
    st.subheader() calls already used throughout this app, splitting
    off the leading emoji automatically, so converting a call site is
    just swapping the function name, not restructuring the string.
    subtitle is optional — most sections don't need one, matching how
    st.subheader() itself never had a second line either.

    Both title and subtitle run through auto_t(), so every section
    heading in the app translates with no call-site changes and no
    dictionary entries — this ONE function covers all 24 of them.
    Applied here rather than at each call site precisely so nothing
    downstream has to know about translation at all.
    """
...
def navigate_to(section_name):
    ...
    Call this, then st.rerun(). The main sidebar nav reads and clears
    this flag directly (see the "with st.sidebar:" block) to set
    _current_nav_section — the app's own state-tracking, not a
    third-party widget's built-in mechanism.
    """
...
def _call_anthropic(messages, max_tokens):
    resp = requests.post(
        ...
        json={"model": ANTHROPIC_MODEL, "max_tokens": max_tokens, "messages": messages},
        timeout=15,
    )
    resp.raise_for_status()
    data = resp.json()
...
def _call_gemini(messages, max_tokens):
    ...
    # messages format used by the other providers — a "contents"
    # array of {"role", "parts": [{"text": ...}]} rather than
    # {"role", "content"}, and it names the model's own prior turns
    # "model", not "assistant" (verified directly against Google's
    # current API docs, not assumed from familiarity with the others).
    gemini_contents = [
    ...
    try:
        resp = requests.post(
            ...
            json={"contents": gemini_contents, "generationConfig": {"maxOutputTokens": max_tokens}},
            timeout=15,
        )
        resp.raise_for_status()
        data = resp.json()
        ...
        # The key rides in the URL for this provider (params={"key": ...}),
        # so raw exception text can contain it verbatim — e.g. an
        # HTTPError's string form includes the full request URL.
        # Redacted HERE, inside the helper, so it's always applied no
        # matter which caller catches this — a failed call must never
        # be the thing that leaks a live key into a log a supervisor
        # can read.
        raise RuntimeError(re.sub(r"key=[^&\s]+", "key=***REDACTED***", f"{type(e).__name__}: {e}")) from e


def _call_groq(messages, max_tokens):
    # Genuinely OpenAI-compatible, not just similar — same
    # request/response shape as the OpenAI helper above, verified
    # directly against Groq's own docs and GitHub repo, not assumed
    # from the marketing claim alone. Only the base URL, key, and
    # model actually differ.
    resp = requests.post(
...
def call_groq_web_search(question, search_settings=None):
    """Answers a question WITH live web search, using Groq's Compound
    Mini model — verified directly against Groq's own docs before
    writing this (console.groq.com/docs/tool-use/built-in-tools/
    web-search). Same endpoint, same auth as regular Groq — the ONLY
    difference is the model string; Compound decides on its own when
    a question actually needs a search versus when it can just answer.

    Deliberately NOT part of the generate_smart_text() fall-through
    chain, and never called automatically — this is genuinely paid
    (Groq's own pricing: $5-8 per 1,000 search requests, on top of
    normal token costs), unlike every other path in this app's AI
    integration, which runs on free tiers. It must only ever fire
    from an explicit, visible opt-in the person chose for that one
    question — silently routing questions through this via a
    provider preference or automatic fallback would risk real,
    unexpected charges for something that looks identical to a free
    request from the person's side.

    Returns (answer, sources) where sources is a list of
    {"title", "url"} dicts extracted from Compound's own citation
    data — or raises on failure, same convention as the other
    provider-calling helpers, so callers handle errors uniformly.

    search_settings, if given, is passed straight through to the API
    as-is — Groq's own include_domains/exclude_domains/country
    controls, unused by default (a global, unrestricted search is
    what "search globally" actually means).
    """
    payload = {
    ...
    resp = requests.post(
        ...
        headers={"Authorization": f"Bearer {GROQ_API_KEY}"},
        json=payload,
        timeout=30,  # search adds real latency beyond a normal completion — longer timeout than the 15s used elsewhere
    )
    resp.raise_for_status()
    data = resp.json()
    answer = data["choices"][0]["message"]["content"]
    sources = []
    executed = data["choices"][0]["message"].get("executed_tools") or []
    for tool_call in executed:
        for result in (tool_call.get("search_results") or {}).get("results", []):
            if result.get("url"):
                sources.append({"title": result.get("title", result["url"]), "url": result["url"]})
    return answer, sources


def _call_openrouter(messages, max_tokens):
    # Also genuinely OpenAI-compatible (verified directly against
    # OpenRouter's own docs) — the one real difference from the
    # Groq/OpenAI helpers is the model string needs a provider prefix
    # (OPENROUTER_MODEL already carries this, e.g. "openai/gpt-4o-mini").
    resp = requests.post(
...
# Display name, the live API key to check, and the calling helper —
# one row per provider. AI_PROVIDER_CHOICES is the subset with a key
# actually configured, in this same default order — built fresh each
# time it's read (not cached at import time) so it always reflects
# current secrets, and is what the Assistant's provider picker in
# Chat is populated from.
_PROVIDER_CHAIN = [
...
def get_configured_ai_providers():
    """Display names of providers that currently have a key set, in
    the same default fall-through order — used to populate the
    Assistant's provider picker with only real, usable choices."""
    _key_values = {"ANTHROPIC_API_KEY": ANTHROPIC_API_KEY, "OPENAI_API_KEY": OPENAI_API_KEY,
                  "GEMINI_API_KEY": GEMINI_API_KEY, "GROQ_API_KEY": GROQ_API_KEY,
                  "OPENROUTER_API_KEY": OPENROUTER_API_KEY}
    return [name for name, key_var, _ in _PROVIDER_CHAIN if _key_values[key_var]]


def generate_smart_text(prompt, max_tokens=300, _return_error=False, preferred_provider=None):
    """Calls whichever AI provider is configured — tries Anthropic,
    OpenAI, Gemini, Groq, then OpenRouter, in that order by default.
    Shared by all AI-assisted features (Smart Work Order Descriptions,
    Incident Severity Prediction, AI Assistant) rather than each
    having its own separate API-calling logic, so a provider or model
    ...
    ...} dicts for genuine multi-turn conversation. All five providers
    accept a messages/contents array in a compatible shape (Gemini's
    is converted inside its own helper; the other four are used as-is).

    `preferred_provider`, if given, is one of the display names from
    get_configured_ai_providers() (e.g. "Groq") — that provider is
    tried FIRST regardless of the default order, matching a person's
    own explicit choice in the Assistant's provider picker. Falls
    through to the rest of the chain in default order if the
    preferred one fails, same as any other provider would.

    Falls through to the next configured provider if one fails,
    rather than stopping at the first one tried — a leftover, broken
    key for an earlier provider in this order (e.g. an old Gemini key
    still set after switching to Groq) must not silently block a
    later, working one from ever being reached. Only returns failure
    once every configured provider has genuinely been attempted;
    _return_error's error string is the LAST failure seen, since
    that's most likely to be the actually-relevant one to fix if
    several providers happen to be misconfigured at once.

    Returns the generated text (str), or None on total failure — a
    down API, a bad key, a network timeout, or nothing configured at
    all. This must never be the thing that blocks someone from
    creating a task or reporting an incident just because a
    third-party AI service had a bad moment; callers fall back to the
    person's own typed input, not this failing outright.

    _return_error=True changes the return to a 3-tuple: (text, error,
    provider_name). provider_name is the display name of whichever
    provider actually answered — this exists specifically so a
    caller can tell the difference between "the preferred provider
    answered" and "the preferred provider failed and this is a
    fallback" — two situations that would otherwise look identical
    from the text alone, which is exactly what made an earlier
    provider-switching bug report hard to diagnose: a failed
    preference silently produces a normal-looking answer from a
    DIFFERENT provider, with nothing in the UI to reveal that.
    """
    messages = prompt if isinstance(prompt, list) else [{"role": "user", "content": prompt}]
    _last_err = "No AI provider configured"

    _key_values = {"ANTHROPIC_API_KEY": ANTHROPIC_API_KEY, "OPENAI_API_KEY": OPENAI_API_KEY,
                  "GEMINI_API_KEY": GEMINI_API_KEY, "GROQ_API_KEY": GROQ_API_KEY,
                  "OPENROUTER_API_KEY": OPENROUTER_API_KEY}
    _chain = list(_PROVIDER_CHAIN)
    if preferred_provider:
        # Move the preferred provider to the front, leaving the
        # relative order of everything else unchanged — a genuine
        # reorder, not a duplicate entry or a second attempt.
        _chain.sort(key=lambda row: row[0] != preferred_provider)

    for _name, _key_var, _call_fn in _chain:
        if not _key_values[_key_var]:
            continue
        try:
            text = _call_fn(messages, max_tokens)
            return (text, "", _name) if _return_error else text
        except Exception as e:
            _last_err = str(e) if isinstance(e, RuntimeError) else f"{type(e).__name__}: {e}"
            log_error(_last_err, endpoint=f"generate_smart_text_{_name.lower()}")

    return (None, _last_err, None) if _return_error else None
...
def ask_maintenance_assistant(question, context_summary, conversation_history=None, preferred_provider=None):
    """Answers questions about the app (grounded in current data and
    the How It Works guide) AND general knowledge questions, with
    genuine multi-turn memory of earlier questions in the same
    session when conversation_history is provided.

    The grounding constraint still applies specifically to app
    questions — never invent a task count, asset name, or feature
    behavior that isn't in the summary. General knowledge questions
    are a different kind of question entirely and are answered from
    the model's own knowledge, same as any general-purpose assistant
    would — the constraint against fabrication is about not
    pretending to know this app's actual data when it doesn't, not
    about refusing unrelated questions.
    ...
    preferred_provider, if given, is passed straight through to
    generate_smart_text() — a person's own explicit choice in the
    Assistant's provider picker, tried first but still falling back
    to the rest of the chain if it fails.

    Returns (answer, provider_name) — provider_name is whichever
    provider actually produced the answer, which may NOT be
    preferred_provider if that one failed and this is a fallback.
    Surfacing this is what lets the UI show "answered via Groq" even
    when Gemini was selected, so a failed preference is visible
    instead of silently indistinguishable from success.
    """
    system_context = (
        "You are the assistant built into a mine's task and asset tracking system "
        "(MWDTS). You handle two kinds of questions:\n"
        "1. Questions about this app's current data or how its features work — for "
        "these, answer using ONLY the data summary and feature guide below. Do not "
        "invent specific numbers, asset names, or task details that aren't in it. If "
        "the summary doesn't contain enough information, say so plainly rather than "
        "guessing.\n"
        "2. General knowledge questions unrelated to the app — answer these normally, "
        "using your own knowledge, the same as any general-purpose assistant would.\n"
        "Keep answers concise (2-4 sentences unless more detail is clearly needed).\n\n"
        f"Current data summary and feature guide:\n{context_summary}"
    )
    ...
    text, _err, provider = generate_smart_text(messages, max_tokens=300,
                                                preferred_provider=preferred_provider, _return_error=True)
...
# -------------------------------
REPORT_DEPARTMENTS = ["Production", "Haulage", "Assets / Maintenance", "Electrical",
                      "Inventory", "Contractors", "Other"]


def generate_worker_report_draft(username, full_name, report_date):
    """Auto-pulls a draft report from real task data — completed tasks
    (completed ON report_date specifically) and currently incomplete
    tasks assigned to this worker (regardless of date, since
    "incomplete" is about current state, not a specific day), plus
    items used and locations derived from those same tasks. The
    worker reviews and adds notes on top rather than retyping
    anything the app already knows.
    ...
    incomplete = []
    for t2 in my_tasks:
        entry = {"task_id": t2["id"], "title": t2.get("title", ""), "location": t2.get("location", "")}
        if t2.get("status") == "Complete":
            completed_at = _parse_dt(t2.get("completed_at"))
            if completed_at and completed_at.date() == report_date:
    ...
    for entry in completed:
        for tp in fetch_task_parts(entry["task_id"]):
            ...
            if part:
                ...
                items_used[name] = items_used.get(name, 0) + float(tp.get("quantity_used", 0) or 0)
    items_used_list = [{"part_name": k, "quantity": v} for k, v in items_used.items()]

    locations = sorted(set(e["location"] for e in (completed + incomplete) if e.get("location")))
...
def create_worker_report(worker_name, username, department, report_date, draft, shift=None, notes=None):
    if not SUPABASE_AVAILABLE:
        ...
        new_report = {
            "id": new_id, "worker_name": worker_name, "username": username, "department": department,
            "report_date": report_date.isoformat(), "shift": shift,
            "completed_tasks": draft["completed_tasks"], "incomplete_tasks": draft["incomplete_tasks"],
            "items_used": draft["items_used"], "locations": draft["locations"], "notes": notes,
            "created_at": datetime.now().isoformat(),
        }
        reports.append(new_report)
        st.session_state.worker_reports_memory = reports
        return new_report
    try:
        res = supabase.table("worker_reports").insert({
            "worker_name": worker_name, "username": username, "department": department,
            "report_date": report_date.isoformat(), "shift": shift,
            "completed_tasks": draft["completed_tasks"], "incomplete_tasks": draft["incomplete_tasks"],
            "items_used": draft["items_used"], "locations": draft["locations"], "notes": notes,
        }).execute()
        if res.data:
            log_audit(worker_name, "worker_report_submitted", {"department": department, "report_date": report_date.isoformat()})
            return res.data[0]
        log_error("worker_reports insert affected 0 rows — likely RLS blocking writes.",
                 endpoint="create_worker_report")
        return None
    except Exception as e:
        log_error(str(e), endpoint="create_worker_report")
        return None
...
def fetch_worker_reports(department=None, report_date=None):
    if not SUPABASE_AVAILABLE:
        ...
        return [r for r in reports
               ...
               and (report_date is None or r["report_date"] == report_date.isoformat())]
    try:
        query = supabase.table("worker_reports").select("*")
        if department:
        ...
        log_error(str(e), endpoint="fetch_worker_reports")
        return []


def build_comprehensive_report(department, report_date):
    """Merges every worker's individual report for a department/date
    into one combined view — the actual point of "comprehensive": one
    document showing everyone's completed/incomplete work, items
    used, and locations for that department that day, not just a
    list of separate reports someone has to read one at a time.
    """
    reports = fetch_worker_reports(department=department, report_date=report_date)
    total_completed = sum(len(r["completed_tasks"]) for r in reports)
    total_incomplete = sum(len(r["incomplete_tasks"]) for r in reports)
    combined_items = {}
    for r in reports:
        for item in r["items_used"]:
            combined_items[item["part_name"]] = combined_items.get(item["part_name"], 0) + item["quantity"]
    all_locations = sorted(set(
        loc.strip() for r in reports for loc in (r.get("locations") or "").split(",") if loc.strip()
    ))
    return {
        "worker_count": len(reports),
        "total_completed": total_completed,
        "total_incomplete": total_incomplete,
        "combined_items_used": [{"part_name": k, "quantity": v} for k, v in
                               sorted(combined_items.items(), key=lambda x: -x[1])],
        "all_locations": all_locations,
        "reports": reports,
    }

# -------------------------------
# 20U. PER-USER AI ASSISTANT CHAT HISTORY
# -------------------------------
def fetch_ai_chat_history(username, limit=50):
    """Returns this user's own chat history, oldest first — never
    another user's. Scoped by username at the application layer
    (this app has no Supabase Auth / server-side identity to enforce
    it at the database layer, same model as every other table here)."""
    if not SUPABASE_AVAILABLE:
        all_msgs = st.session_state.get("ai_chat_memory", [])
        return [m for m in all_msgs if m["username"] == username][-limit:]
    try:
        res = (supabase.table("ai_chat_messages").select("*")
              .eq("username", username).order("created_at", desc=True).limit(limit).execute())
        return list(reversed(res.data or []))
    except Exception as e:
        log_error(str(e), endpoint="fetch_ai_chat_history")
        return []


def save_ai_chat_message(username, role, content):
    if not SUPABASE_AVAILABLE:
        all_msgs = st.session_state.get("ai_chat_memory", [])
        all_msgs.append({"username": username, "role": role, "content": content,
                        "created_at": datetime.now().isoformat()})
        st.session_state.ai_chat_memory = all_msgs
        return True
    try:
        res = supabase.table("ai_chat_messages").insert({
            "username": username, "role": role, "content": content,
        }).execute()
        return bool(res.data)
    except Exception as e:
        log_error(str(e), endpoint="save_ai_chat_message")
        return False


def clear_ai_chat_history(username):
    """Deletes only this user's own messages — the query is scoped
    by username, so this can never touch another user's history."""
    if not SUPABASE_AVAILABLE:
        all_msgs = st.session_state.get("ai_chat_memory", [])
        st.session_state.ai_chat_memory = [m for m in all_msgs if m["username"] != username]
        return True
    try:
        supabase.table("ai_chat_messages").delete().eq("username", username).execute()
        return True
    except Exception as e:
        log_error(str(e), endpoint="clear_ai_chat_history")
        return False
...
# AI Assistant chatbot draws on this same content when
...
HOW_IT_WORKS_GUIDE = {
    ...
    "Communication & Admin": {
        ...
        "Chat": "Global, Supervisor-only, and private messaging, plus an AI Assistant room "
            "(if AI features are configured) that answers questions about the app's current "
            "data, and general knowledge questions too. Each person's AI conversation is "
            "private and saved just for them — no one else can see it.",
...
# =====================================================================
# NAV_CATEGORIES — sidebar grouping, so 30+ flat items become 6
# collapsible sections instead. Covers every possible nav_options
# value (including role-gated ones like "Owner Console" that only
# some users ever see) — rendering filters each category down to
# only the items actually present in that user's final, already-
...
# section can be expanded by default rather than collapsed and hiding
# where you already are.
SECTION_TO_CATEGORY = {section: cat for cat, sections in NAV_CATEGORIES.items() for section in sections}
...
SUPPORTED_LANGUAGES = {
    ...
    # Listed by their own endonyms, as speakers actually refer to
    # them. Both are Akan dialects widely spoken in Ghana — Twi here
    # is Asante Twi.
    "tw": "Twi", "fat": "Fante",
}
...
TRANSLATIONS = {
    ...
    "tw": {
        ...
        "nav.Handover": "Adwuma Dan",
        "nav.Worker Reports": "Adwumayɛfoɔ Amanneɛbɔ",
        "nav.Contractors": "Kɔntrakta Foɔ",
        "nav.Analytics": "Nhwehwɛmu",
        "nav.Chat": "Nkɔmmɔ",
        "nav.Feedback": "Nsusuiɛ",
        "nav.Admin": "Sohwɛ",
        "nav.Profile": "Wo Ho Nsɛm",
        "nav.Timeline": "Nneɛma A Asisi",
        "nav.About": "Fa Ho",
        "nav.Production": "Nneɛma Yɛ",
        "nav.Haulage": "Nneɛma Soa",
        "nav.Wallboard": "Ɔfasuo Bɔɔd",
        "nav.Crew Clock": "Adwumayɛfoɔ Dɔn",
        "nav.JSA Library": "JSA Nwoma Dan",
        "nav.Job Plans": "Adwuma Nhyehyɛeɛ",
        "nav.Locations": "Mmeaeɛ",
        "nav.Electrical Overview": "Anyinam Ahoɔden Mu Nhwɛ",
        "nav.Motor Rewinds": "Mota Nkyekyerɛ Foforɔ",
        "nav.Instrument Calibration": "Mfidie Nhyehyɛ (Calibration)",
        "nav.Outage Commander": "Kanea Dum Sohwɛfoɔ",
        "nav.Transformer Health": "Transformer Apɔmuden",
        "nav.Fault Recorder": "Mfomsoɔ Kyerɛwtohɔ",
        "nav.HV Switching Schedule": "HV Nsakraeɛ Nhyehyɛeɛ",
        "nav.Relay Settings": "Relay Nhyehyɛeɛ",
        "nav.Arc Flash Studies": "Arc Flash Nhwehwɛmu",
        "nav.Technician Certifications": "Adwumayɛfoɔ Nkrataa (Certifications)",
        "incidents.title": "🚨 Nsɛnnennen ne Ahobammɔ Amanneɛbɔ",
        "incidents.tab_all": "Nsɛnnennen Nyinaa",
        "incidents.tab_report": "Bɔ Amanneɛ",
        "incidents.tab_my_reports": "M'Amanneɛbɔ",
        "incidents.empty_title": "Wɔnbɔɔ nsɛnnennen biara amanneɛ",
        "incidents.empty_desc": "Ɛyɛ nsɛnkyerɛnneɛ pa — ɛha na asiane ne mpaepaeɛ amanneɛbɔ bɛpue.",
        "incidents.export_csv": "📥 Yi Nsɛnnennen sɛ CSV",
        "incidents.download_csv": "Twe CSV",
        "incidents.search_placeholder": "🔍 Hwehwɛ ɛkwan, beaeɛ, anaa nkyerɛkyerɛmu so",
        "incidents.reported_by": "{name} na ɔbɔɔ amanneɛ",
        "incidents.id_no": "ID {no}",
        "incidents.paper_ref": "Krataa ref #{no}",
        "incidents.field_immediate_action": "Ntɛm adeyɛ",
        "incidents.field_reporter_suggestion": "Amanneɛbɔfoɔ nsusuiɛ",
        "incidents.field_root_cause": "Ase nnyinasoɔ",
        "incidents.field_corrective_action": "Nsiesieɛ adeyɛ",
        "incidents.acknowledged_by": "{name} gye toom wɔ {time}",
        "incidents.acknowledge_btn": "✋ Gye toom — #{id}",
        "incidents.acknowledged_success": "Woagye atom. Seesei wone amanneɛbɔ yi wura.",
        "incidents.update_failed": "Nsakraeɛ antumi anyɛ adwuma. Sɛ ɛkɔ so a, ebia Row Level Security resi nkyerɛw kwan wɔ incidents table no so — hwɛ schema_additions.sql.",
        "incidents.investigate_expander": "⚙️ Nhwehwɛmu #{id}",
        "incidents.status_label": "Tebea",
        "incidents.root_cause_label": "Ase Nnyinasoɔ",
        "incidents.corrective_action_label": "Nsiesieɛ Adeyɛ",
        "incidents.save_investigation": "💾 Kora Nhwehwɛmu So",
        "incidents.updated_success": "Wɔasesa nsɛnnennen no.",
        "incidents.submit_new_heading": "Fa Nsɛnnennen Amanneɛbɔ Foforɔ Kɔ",
        "incidents.submit_caption": "Bɔ mpaepaeɛ, apirakuro, ne asiane ho amanneɛ ntɛm ara. Amanneɛbɔ a ɛyɛ Critical/High nyinaa bɔ sohwɛfoɔ amanneɛ ntɛm.",
        "incidents.report_details_heading": "Amanneɛbɔ nsɛm",
        "incidents.type_label": "Ɛkwan",
        "incidents.severity_label": "Ɛmu Duru",
        "incidents.department_label": "Dwumadibea",
        "incidents.shift_label": "Adwuma Berɛ",
        "incidents.your_id_label": "Wo ID No.",
        "incidents.related_asset_label": "Mfidie A Ɛfa Ho (sɛ ɛwɔ hɔ a)",
        "incidents.none_option": "Ɛnni hɔ",
        "incidents.witnesses_label": "Adansefoɔ (sɛ ɛwɔ hɔ a)",
        "incidents.paper_ref_label": "Krataa nwoma ref. no. (sɛ ɛwɔ hɔ a)",
        "incidents.paper_ref_placeholder": "te sɛ 0000651",
        "incidents.paper_ref_help": "Sɛ wɔdii kan twerɛɛ yei wɔ krataa asiane/mpaepaeɛ nwoma no mu a, twerɛ ne nɔma wɔ ha na yɛatumi de mmienu no atoto ho.",
        "incidents.location_label": "Beaeɛ / Mpɔtam *",
        "incidents.description_label": "Nkyerɛkyerɛmu *",
        "incidents.description_placeholder": "Ɛdeɛn na ɛsiiɛ? Ka no pefee.",
        "incidents.immediate_action_label": "Ntɛm Adeyɛ A Wɔyɛeɛ",
        "incidents.immediate_action_placeholder": "Ɛdeɛn na wɔyɛɛ no ntɛm ara?",
        "incidents.suggestion_label": "Me nsusuiɛ / nsiesieɛ adeyɛ",
        "incidents.suggestion_placeholder": "Wosusu sɛ ɛdeɛn na ɛsɛ sɛ wɔyɛ na yei ansi bio?",
        "incidents.suggestion_help": "W'ankasa wo nsusuiɛ wɔ berɛ a worebɔ amanneɛ — ɛnyɛ deɛ sohwɛfoɔ a ɔyɛ nhwehwɛmu no bɛsi gyinaeɛ akyire yi.",
        "incidents.confirm_checkbox": "Megye tom sɛ nsɛm a ɛwɔ soro hɔ yɛ nokorɛ sɛdeɛ menim",
        "incidents.confirm_help": "Ɛte sɛ deɛ wode wo nsa hyɛ krataa amanneɛbɔ ase.",
        "incidents.submit_btn": "🚨 Fa Amanneɛbɔ Kɔ",
        "incidents.err_required": "Beaeɛ ne Nkyerɛkyerɛmu ho hia.",
        "incidents.err_confirm": "Yɛsrɛ wo gye tom sɛ nsɛm no yɛ nokorɛ ansa na wode akɔ.",
        "incidents.success_reported": "Wɔabɔ nsɛnnennen no amanneɛ. Meda wo ase sɛ woma beaeɛ ha yɛ ahobammɔ.",
        "incidents.warn_flagged": "Wɔde yei ato sohwɛfoɔ anim sɛ ɔnhwɛ ntɛm ara.",
        "incidents.err_submit_failed": "Amanneɛbɔ no antumi ankɔ.",
        "permits.title": "🔐 Adwuma Kwan Krataa / LOTO Nkrataa",
        "permits.caption": "Ɛsɛ sɛ wɔma kwan krataa, na onipa a ɔreyɛ adwuma no gye tom, na sɛ wɔwie a ɔde ne nsa hyɛ ase bio. Nkrataa yi ne saa nkabom no ho adanseɛ.",
        "permits.tab_active": "Kwan Nkrataa A Ɛreyɛ Adwuma",
        "permits.tab_issue": "Ma Kwan Krataa",
        "permits.tab_history": "Kwan Krataa Abakɔsɛm",
        "permits.field_task": "Adwuma",
        "permits.field_lock_tags": "Lock tags",
        "permits.field_isolation_points": "Isolation points",
        "permits.field_hazards": "Asiane",
        "permits.issued_by": "{name} na ɔmaeɛ",
        "permits.accepted_by": "{name} na ɔgye toomeɛ",
        "permits.signed_back_by": "{name} na ɔde ne nsa hyɛɛ ase bio",
        "permits.valid_until": "Ɛyɛ adwuma kɔsi {time}",
        "permits.step_issued": "Wɔamaeɛ",
        "permits.step_accepted": "Wɔagye Atom",
        "permits.step_signed_back": "Wɔde Nsa Ahyɛ Ase Bio",
        "permits.expired_badge": "ATWAM",
        "permits.accept_btn": "✍️ Gye Isolation Toom",
        "permits.accept_success": "Wɔagye kwan krataa no atom. Seesei wone obi a asɛyɛdeɛ da wo so.",
        "permits.accept_failed": "Gye toom antumi anyɛ adwuma — wɔansesa kwan krataa no. Hwɛ Row Level Security wɔ permits table no so ansa na woagye adi sɛ isolation wɔ hɔ.",
        "permits.signback_btn": "✅ Fa Nsa Hyɛ Ase Bio",
        "permits.signback_success": "Wɔde nsa ahyɛ kwan krataa no ase na wɔato mu.",
        "permits.signback_failed": "Nsa a wode bɛhyɛ ase antumi anyɛ adwuma — kwan krataa no da so kyerɛ sɛ ɛreyɛ adwuma.",
        "permits.cancel_btn": "🚫 Gyae",
        "permits.cancel_failed": "Gyae antumi anyɛ adwuma — wɔansesa kwan krataa no.",
        "permits.empty_title": "Kwan krataa biara nni hɔ a abue",
        "permits.empty_desc": "Adwuma Kwan Krataa / LOTO nkrataa bɛpue wɔ ha sɛ wɔma bi a.",
        "permits.expired_warning": "⚠️ Kwan nkrataa {n} atwam wɔn berɛ na ɛsɛ sɛ wɔhwɛ anaa wɔgyae.",
        "permits.search_placeholder": "🔍 Hwehwɛ kwan krataa ɛkwan anaa lock tag so",
        "permits.issue_new_heading": "Ma Kwan Krataa Foforɔ",
        "permits.no_open_tasks": "Adwuma biara nni hɔ a abue a wɔde kwan krataa bɛka ho.",
        "permits.task_label": "Adwuma a ɛhia kwan krataa *",
        "permits.type_label": "Kwan Krataa Ɛkwan",
        "permits.lock_tag_label": "Lock / Tag Nɔma *",
        "permits.lock_tag_placeholder": "te sɛ LT-1042, LT-1043",
        "permits.isolation_points_label": "Isolation Points *",
        "permits.isolation_points_placeholder": "Twerɛ ahoɔden nnyinasoɔ biara a wɔatwa mu",
        "permits.hazards_label": "Asiane A Wɔahunu *",
        "permits.hazards_placeholder": "Ahoɔden a asie, nhyɛsoɔ a aka, ne nkaeɛ",
        "permits.valid_hours_label": "Ɛyɛ adwuma (nnɔnhwerew)",
        "permits.confirm_checkbox": "Megye tom sɛ wɔde nsa ahwɛ isolation wɔ beaeɛ biara a wɔabobɔ din wɔ soro hɔ.",
        "permits.issue_btn": "🔐 Ma Kwan Krataa",
        "permits.err_required": "Lock tags, isolation points, ne asiane nyinaa ho hia.",
        "permits.err_confirm": "Ɛsɛ sɛ wogye tom sɛ wɔde nsa ahwɛ isolation ansa na wɔama kwan krataa.",
        "permits.issue_success": "Wɔama kwan krataa #{id}. Seesei ɛsɛ sɛ onipa a ɔreyɛ adwuma no gye tom.",
        "permits.issue_failed": "Kwan krataa no antumi amma.",
        "permits.no_closed": "Kwan krataa biara nni hɔ a wɔato mu.",
        "contractors.title": "👷 Kɔntrakta Foɔ Sohwɛ",
        "contractors.caption": "Wɔhwɛ induction ne insurance berɛ a ɛtwam ɛfiri sɛ ɛtaa si beaeɛ ha kwan. Nkrataa a atwam anaa ɛnni hɔ no, wɔkyerɛ sɛ ɛsi kwan.",
        "contractors.tab_all": "Kɔntrakta Foɔ Nyinaa",
        "contractors.tab_add": "Fa Kɔntrakta Ka Ho",
        "contractors.blocking_warning": "🚫 Kɔntrakta {n} wɔ nkrataa a atwam anaa ɛnni hɔ na ɛnsɛ sɛ wɔma wɔn kwan ba beaeɛ ha.",
        "contractors.empty_title": "Wɔntwerɛɛ kɔntrakta biara din",
        "contractors.empty_desc": "Fa kɔntrakta ka ho na fi ase hwɛ induction ne insurance.",
        "contractors.search_placeholder": "🔍 Hwehwɛ adwumakuo anaa onipa din so",
        "contractors.not_set": "Wɔnhyɛeɛ",
        "contractors.field_induction_expires": "Induction twam",
        "contractors.field_insurance_expires": "Insurance twam",
        "contractors.field_competencies": "Nimdeɛ",
        "contractors.update_expander": "⚙️ Sesa {name}",
        "contractors.induction_expiry_label": "Induction berɛ twam",
        "contractors.insurance_expiry_label": "Insurance berɛ twam",
        "contractors.save_btn": "💾 Kora So",
        "contractors.update_success": "Wɔasesa kɔntrakta no.",
        "contractors.save_failed": "Ankora so — WƆANSESA nkrataa nna no. Hwɛ Row Level Security wɔ contractors table no so ansa na woagye adi sɛ kɔntrakta yi tebea yɛ foforɔ.",
        "contractors.company_name_label": "Adwumakuo Din *",
        "contractors.contact_person_label": "Onipa A Yɛbɛfrɛ No",
        "contractors.contact_email_label": "Email",
        "contractors.contact_phone_label": "Telefon",
        "contractors.induction_date_label": "Induction Da",
        "contractors.induction_expiry_cap_label": "Induction Berɛ Twam",
        "contractors.insurance_expiry_cap_label": "Insurance Berɛ Twam",
        "contractors.competencies_label": "Nimdeɛ / Nkrataa (Certifications)",
        "contractors.competencies_placeholder": "te sɛ Confined space, EWP licence, HV switching",
        "contractors.notes_label": "Nsɛm A Wɔatwerɛ",
        "contractors.add_btn": "➕ Fa Kɔntrakta Ka Ho",
        "contractors.add_success": "Wɔde kɔntrakta '{name}' aka ho.",
        "contractors.add_failed": "Kɔntrakta no antumi anka ho.",
        "contractors.err_name_required": "Adwumakuo Din ho hia.",
        "nav.Owner Console": "Owura Sohwɛbea",
        "common.save": "Kora So",
        "common.cancel": "Gyae",
        "common.submit": "Fa Kɔ",
        "common.delete": "Pepa",
        ...
        "task.btn_post_comment": "Fa Nsɛm Kɔ",
        "task.btn_upload_attachment": "Fa Krataa Kɔ",
        "task.btn_upload": "Fa Kɔ",
        "task.btn_export_csv": "📥 Yi Nnwuma sɛ CSV",
        "task.caption_full_breakdowns": "Nkyekyɛmu a ɛdi mũ, Pareto nhwehwɛmu, ne ka ho amanneɛbɔ wɔ **Analytics** fa mu.",
        "task.caption_no_attachments": "Nkrataa biara nni hɔ.",
        "task.caption_no_broadcasts": "Amanneɛbɔ biara nni hɔ.",
        "task.caption_no_comments": "Nsɛm biara nni hɔ.",
        "task.chk_recurring": "Adwuma A Ɛba Daa (Preventive Maintenance)",
        "task.chk_requires_jsa": "Ɛhia JSA",
        "task.chk_requires_loto": "Ɛhia LOTO",
        "task.chk_jsa_signed": "📋 Wɔde Nsa Ahyɛ JSA Ase",
        "task.share_link_label": "Kyɛ Link",
        "task.chk_loto_isolated": "🔒 Wɔatwa LOTO Mu",
        "task.err_cannot_move_progress": "Wontumi mfa nkɔ 'In Progress' a kwan krataa a wɔagye atom nni hɔ.",
        "task.err_delete_failed": "Pepa antumi anyɛ adwuma. Sɛ ɛkɔ so a, ebia Row Level Security resi nkyerɛw kwan wɔ tasks table no so.",
        "task.err_create_failed": "Adwuma no antumi amma.",
        "task.err_comment_failed": "Nsɛm no antumi ankɔ.",
        "task.err_title_location_required": "Din ne Beaeɛ ho hia.",
        "task.err_upload_failed": "Antumi amfa ankɔ.",
        "task.err_safety_forms_required": "🔒 Ahobammɔ isolation nkrataa ho hia ansa na woakɔ so.",
        "task.err_permit_required": "🚫 **Adwuma yi hia Adwuma Kwan Krataa a wɔagye atom.** Kwan krataa biara nni hɔ a wɔatwerɛ ato so. Ka kyerɛ wo sohwɛfoɔ na ɔmma bi, na gye tom wɔ Kwan Nkrataa fa mu ansa na wafi adwuma ase.",
        "task.info_no_active_users": "Wɔn a wɔdi dwuma biara nni hɔ.",
        "task.info_no_data": "Nsɛm biara nni hɔ a yɛbɛkyerɛ.",
        "task.info_no_messages": "Wɔnsomaa nkra biara.",
        "task.info_no_tasks_assigned": "Wɔmfaa adwuma biara mmaa wo.",
        "task.info_no_tasks_found": "Yɛanhu adwuma biara.",
        "task.info_no_tasks_manage": "Adwuma biara nni hɔ a wobɛhwɛ so.",
        "task.info_readonly_directory": "Yei yɛ nkrataa a wobɛtumi akenkan nko ara. Kwan a wɔma, dibea nsakraeɛ, ne nsiyaeɛ deɛ, akawnt wura na ɔhwɛ so.",
        "task.info_owner_note": "Wone owura — kwan a wɔma ne dibea nsakraeɛ wɔ **Owner Console → Access Requests** mu.",
        "task.info_latest_broadcasts": "📢 Amanneɛbɔ A Ɛtwa Toɔ:",
        "task.hdr_all_broadcasts": "Amanneɛbɔ Nkra Nyinaa",
        "task.hdr_all_tasks": "Nsiesie Nnwuma Nyinaa",
        "task.hdr_dispatch_new": "Fa Adwuma Tikiti Foforɔ Kɔ",
        "task.hdr_full_control": "Adwuma So Tumi Nyinaa",
        "task.hdr_recent_broadcasts": "Amanneɛbɔ A Ɛbaa Nnansa Yi",
        "task.hdr_user_directory": "👥 Wɔn A Wɔdi Dwuma",
        "task.hdr_task_analytics": "📊 Adwuma Nhwehwɛmu",
        "task.hdr_active_users": "Wɔn a wɔdi dwuma",
        "task.hdr_suspended": "Wɔasi wɔn kwan",
        "task.hdr_kpis": "🎯 Adwuma Su Nsusuiɛ (KPIs)",
        "task.txt_closeout_details": "**Adwuma wieeɛ nsɛm** — yeinom na ɛma nokwaredie ne ka ho amanneɛbɔ.",
        "task.txt_already_uploaded": "**📸 Deɛ wɔde akɔ dada:**",
        "task.field_failure_code": "Sɛeɛ nɔma (failure code)",
        "task.field_linked_asset": "Mfidie A Ɛfa Ho (sɛ ɛwɔ hɔ a)",
        "task.field_priority": "Deɛ Ɛdi Kan",
        "task.field_recurrence": "Ɛba Daa Ɛkwan",
        "task.field_update_status": "Sesa Tebea",
        "task.field_work_type": "Adwuma Ɛkwan",
        "task.success_attachment": "Wɔde krataa no akɔ!",
        "task.success_photo": "Wɔde mfonini no akɔ!",
        "task.success_safety_checks": "✅ Ahobammɔ nhwehwɛ no adi nkonim.",
        "task.success_no_unassigned": "🎉 Adwuma biara nni hɔ a wɔmfaa mmaa obi seesei.",
        "task.field_add_comment": "Fa nsɛm ka ho",
        "task.field_location": "Beaeɛ / Mpɔtam *",
        "task.field_task_title": "Adwuma Din *",
    ...
    "fat": {
        ...
        "nav.Assets": "Mfidzie",
        "nav.Permits": "Kwan Krataa (Permits)",
        "nav.Inventory": "Ndzɛmba Sie",
        "nav.Incidents": "Nsɛnnehunu (Incidents)",
        "nav.Handover": "Edwuma Dan",
        "nav.Worker Reports": "Edwumayɛfo Amanebɔ",
        "nav.Contractors": "Kɔntrakta Fo",
        "nav.Analytics": "Nhwehwɛmu",
        "nav.Chat": "Nkɔmbɔ",
        "nav.Feedback": "Nsusuwii",
        "nav.Admin": "Nhwɛso",
        "nav.Profile": "Wo Ho Nsɛm",
        "nav.Timeline": "Ndzɛmba a Asi",
        "nav.About": "Fa Ho",
        "nav.Production": "Ndzɛmba Yɛ",
        "nav.Haulage": "Ndzɛmba Soa",
        "nav.Wallboard": "Ɔfasu Bɔɔd",
        "nav.Crew Clock": "Edwumayɛfo Dɔn",
        "nav.JSA Library": "JSA Nwoma Dan",
        "nav.Job Plans": "Edwuma Nhyehyɛɛ",
        "nav.Locations": "Mbeae",
        "nav.Electrical Overview": "Anyinam Ahoɔdzen Mu Nhwɛ",
        "nav.Motor Rewinds": "Mota Nkyekyerɛ Fofor",
        "nav.Instrument Calibration": "Mfidzie Nhyehyɛ (Calibration)",
        "nav.Outage Commander": "Kanyaa Dum Nhwɛsofo",
        "nav.Transformer Health": "Transformer Apɔmudzen",
        "nav.Fault Recorder": "Mfomso Kyerɛwtohɔ",
        "nav.HV Switching Schedule": "HV Nsakyeraa Nhyehyɛɛ",
        "nav.Relay Settings": "Relay Nhyehyɛɛ",
        "nav.Arc Flash Studies": "Arc Flash Nhwehwɛmu",
        "nav.Technician Certifications": "Edwumayɛfo Nkrataa (Certifications)",
        "incidents.title": "🚨 Nsɛnnehunu na Ahobammɔ Amanebɔ",
        "incidents.tab_all": "Nsɛnnehunu Nyinara",
        "incidents.tab_report": "Bɔ Amanebɔ",
        "incidents.tab_my_reports": "M'Amanebɔ",
        "incidents.empty_title": "Wɔmmbɔɔ nsɛnnehunu biara amanebɔ",
        "incidents.empty_desc": "Ɔyɛ nsɛnkyerɛdze pa — ɛha na asiandze na mpaapaa amanebɔ bepue.",
        "incidents.export_csv": "📥 Yi Nsɛnnehunu dɛ CSV",
        "incidents.download_csv": "Twe CSV",
        "incidents.search_placeholder": "🔍 Hwehwɛ ekwan, bea, anaa nkyerɛkyerɛmu do",
        "incidents.reported_by": "{name} na ɔbɔɔ amanebɔ",
        "incidents.id_no": "ID {no}",
        "incidents.paper_ref": "Krataa ref #{no}",
        "incidents.field_immediate_action": "Ntsɛm adzeyɛ",
        "incidents.field_reporter_suggestion": "Amanebɔnyi nsusuwii",
        "incidents.field_root_cause": "Ase nnyinaso",
        "incidents.field_corrective_action": "Nsiesie adzeyɛ",
        "incidents.acknowledged_by": "{name} gyee tom wɔ {time}",
        "incidents.acknowledge_btn": "✋ Gye tom — #{id}",
        "incidents.acknowledged_success": "Ayɛ gye atom. Afei wonye amanebɔ yi wura.",
        "incidents.update_failed": "Nsakyeraa antum anyɛ edwuma. Sɛ ɔkɔ do a, ebia Row Level Security resi nkyerɛw kwan wɔ incidents table no do — hwɛ schema_additions.sql.",
        "incidents.investigate_expander": "⚙️ Nhwehwɛmu #{id}",
        "incidents.status_label": "Tsebea",
        "incidents.root_cause_label": "Ase Nnyinaso",
        "incidents.corrective_action_label": "Nsiesie Adzeyɛ",
        "incidents.save_investigation": "💾 Kora Nhwehwɛmu Do",
        "incidents.updated_success": "Wɔasesa nsɛnnehunu no.",
        "incidents.submit_new_heading": "Fa Nsɛnnehunu Amanebɔ Fofor Kɔ",
        "incidents.submit_caption": "Bɔ mpaapaa, apirakuru, na asiandze ho amanebɔ ntsɛm ara. Amanebɔ a ɔyɛ Critical/High nyinara bɔ nhwɛsofo amanebɔ ntsɛm.",
        "incidents.report_details_heading": "Amanebɔ nsɛm",
        "incidents.type_label": "Ekwan",
        "incidents.severity_label": "Mu Duru",
        "incidents.department_label": "Edwumadzibea",
        "incidents.shift_label": "Edwuma Ber",
        "incidents.your_id_label": "Wo ID No.",
        "incidents.related_asset_label": "Mfidzie a Ɔfa Ho (sɛ ɔwɔ hɔ a)",
        "incidents.none_option": "Ɔnnyi hɔ",
        "incidents.witnesses_label": "Adanseefo (sɛ ɔwɔ hɔ a)",
        "incidents.paper_ref_label": "Krataa nwoma ref. no. (sɛ ɔwɔ hɔ a)",
        "incidents.paper_ref_placeholder": "dɛ 0000651",
        "incidents.paper_ref_help": "Sɛ wodzii kan kyerɛw yi wɔ krataa asiandze/mpaapaa nwoma no mu a, kyerɛw ne nɔma wɔ ha ama yeetum dze mmienu no atoto ho.",
        "incidents.location_label": "Bea / Mpɔtam *",
        "incidents.description_label": "Nkyerɛkyerɛmu *",
        "incidents.description_placeholder": "Dɛn na ɔsii? Ka no pefee.",
        "incidents.immediate_action_label": "Ntsɛm Adzeyɛ a Wɔyɛe",
        "incidents.immediate_action_placeholder": "Dɛn na wɔyɛɛ no ntsɛm ara?",
        "incidents.suggestion_label": "Me nsusuwii / nsiesie adzeyɛ",
        "incidents.suggestion_placeholder": "Osusu dɛ dɛn na ɔsɛ dɛ wɔyɛ ama yi ansi bio?",
        "incidents.suggestion_help": "W'ankasa wo nsusuwii wɔ ber a oroobɔ amanebɔ — ɔnnyɛ dza nhwɛsofo a ɔyɛ nhwehwɛmu no besi gyinae ekyir yi.",
        "incidents.confirm_checkbox": "Migye tom dɛ nsɛm a ɔwɔ do hɔ yɛ nokwar dɛ mbrɛ minyim",
        "incidents.confirm_help": "Ɔtse dɛ dza wodze wo nsa hyɛ krataa amanebɔ ase.",
        "incidents.submit_btn": "🚨 Fa Amanebɔ Kɔ",
        "incidents.err_required": "Bea na Nkyerɛkyerɛmu ho hia.",
        "incidents.err_confirm": "Yɛsrɛ wo gye tom dɛ nsɛm no yɛ nokwar ansaana wodze akɔ.",
        "incidents.success_reported": "Wɔabɔ nsɛnnehunu no amanebɔ. Meda wo ase dɛ ama bea ha yɛ ahobammɔ.",
        "incidents.warn_flagged": "Wɔdze yi ato nhwɛsofo enyim dɛ ɔnhwɛ ntsɛm ara.",
        "incidents.err_submit_failed": "Amanebɔ no antum ankɔ.",
        "permits.title": "🔐 Edwuma Kwan Krataa / LOTO Nkrataa",
        "permits.caption": "Ɔsɛ dɛ wɔma kwan krataa, na nyimpa a ɔroyɛ edwuma no gye tom, na sɛ wowie a ɔdze ne nsa hyɛ ase bio. Nkrataa yi nye dɛm nkabom no ho adanse.",
        "permits.tab_active": "Kwan Nkrataa a Ɔroyɛ Edwuma",
        "permits.tab_issue": "Ma Kwan Krataa",
        "permits.tab_history": "Kwan Krataa Abakɔsɛm",
        "permits.field_task": "Edwuma",
        "permits.field_lock_tags": "Lock tags",
        "permits.field_isolation_points": "Isolation points",
        "permits.field_hazards": "Asiandze",
        "permits.issued_by": "{name} na ɔmae",
        "permits.accepted_by": "{name} na ɔgyee tom",
        "permits.signed_back_by": "{name} na ɔdze ne nsa hyɛɛ ase bio",
        "permits.valid_until": "Ɔyɛ edwuma kosi {time}",
        "permits.step_issued": "Wɔamae",
        "permits.step_accepted": "Wɔagye Atom",
        "permits.step_signed_back": "Wɔdze Nsa Ahyɛ Ase Bio",
        "permits.expired_badge": "ATWAM",
        "permits.accept_btn": "✍️ Gye Isolation Tom",
        "permits.accept_success": "Wɔagye kwan krataa no atom. Afei wonye obi a asɛyɛdze da wo do.",
        "permits.accept_failed": "Gye tom antum anyɛ edwuma — wɔansesa kwan krataa no. Hwɛ Row Level Security wɔ permits table no do ansaana ebegye edzi dɛ isolation wɔ hɔ.",
        "permits.signback_btn": "✅ Fa Nsa Hyɛ Ase Bio",
        "permits.signback_success": "Wɔdze nsa ahyɛ kwan krataa no ase na wɔato mu.",
        "permits.signback_failed": "Nsa a wubedze ahyɛ ase antum anyɛ edwuma — kwan krataa no da do kyerɛ dɛ ɔroyɛ edwuma.",
        "permits.cancel_btn": "🚫 Gyaa",
        "permits.cancel_failed": "Gyaa antum anyɛ edwuma — wɔansesa kwan krataa no.",
        "permits.empty_title": "Kwan krataa biara nnyi hɔ a abue",
        "permits.empty_desc": "Edwuma Kwan Krataa / LOTO nkrataa bepue wɔ ha sɛ wɔma bi a.",
        "permits.expired_warning": "⚠️ Kwan nkrataa {n} atwam hɔn ber na ɔsɛ dɛ wɔhwɛ anaa wogyaa.",
        "permits.search_placeholder": "🔍 Hwehwɛ kwan krataa ekwan anaa lock tag do",
        "permits.issue_new_heading": "Ma Kwan Krataa Fofor",
        "permits.no_open_tasks": "Edwuma biara nnyi hɔ a abue a wobedze kwan krataa aka ho.",
        "permits.task_label": "Edwuma a ohia kwan krataa *",
        "permits.type_label": "Kwan Krataa Ekwan",
        "permits.lock_tag_label": "Lock / Tag Nɔma *",
        "permits.lock_tag_placeholder": "dɛ LT-1042, LT-1043",
        "permits.isolation_points_label": "Isolation Points *",
        "permits.isolation_points_placeholder": "Kyerɛw ahoɔdzen nnyinaso biara a wɔatwa mu",
        "permits.hazards_label": "Asiandze a Wɔahu *",
        "permits.hazards_placeholder": "Ahoɔdzen a asie, nhyɛso a aka, na nkae",
        "permits.valid_hours_label": "Ɔyɛ edwuma (nnɔnhwer)",
        "permits.confirm_checkbox": "Migye tom dɛ wɔdze nsa ahwɛ isolation wɔ bea biara a wɔabobɔ dzin wɔ do hɔ.",
        "permits.issue_btn": "🔐 Ma Kwan Krataa",
        "permits.err_required": "Lock tags, isolation points, na asiandze nyinara ho hia.",
        "permits.err_confirm": "Ɔsɛ dɛ egye tom dɛ wɔdze nsa ahwɛ isolation ansaana wɔama kwan krataa.",
        "permits.issue_success": "Wɔama kwan krataa #{id}. Afei ɔsɛ dɛ nyimpa a ɔroyɛ edwuma no gye tom.",
        "permits.issue_failed": "Kwan krataa no antum amma.",
        "permits.no_closed": "Kwan krataa biara nnyi hɔ a wɔato mu.",
        "contractors.title": "👷 Kɔntrakta Fo Nhwɛso",
        "contractors.caption": "Wɔhwɛ induction na insurance ber a ɔtwam osiandɛ ɔtaa si bea ha kwan. Nkrataa a atwam anaa ɔnnyi hɔ no, wɔkyerɛ dɛ osi kwan.",
        "contractors.tab_all": "Kɔntrakta Fo Nyinara",
        "contractors.tab_add": "Fa Kɔntrakta Ka Ho",
        "contractors.blocking_warning": "🚫 Kɔntrakta {n} wɔ nkrataa a atwam anaa ɔnnyi hɔ na ɔnnsɛ dɛ wɔma hɔn kwan ba bea ha.",
        "contractors.empty_title": "Wɔankyerɛw kɔntrakta biara dzin",
        "contractors.empty_desc": "Fa kɔntrakta ka ho na fi ase hwɛ induction na insurance.",
        "contractors.search_placeholder": "🔍 Hwehwɛ edwumakuw anaa nyimpa dzin do",
        "contractors.not_set": "Wɔanhyɛ",
        "contractors.field_induction_expires": "Induction twam",
        "contractors.field_insurance_expires": "Insurance twam",
        "contractors.field_competencies": "Nyimdzee",
        "contractors.update_expander": "⚙️ Sesa {name}",
        "contractors.induction_expiry_label": "Induction ber twam",
        "contractors.insurance_expiry_label": "Insurance ber twam",
        "contractors.save_btn": "💾 Kora Do",
        "contractors.update_success": "Wɔasesa kɔntrakta no.",
        "contractors.save_failed": "Ankora do — WƆANSESA nkrataa nna no. Hwɛ Row Level Security wɔ contractors table no do ansaana ebegye edzi dɛ kɔntrakta yi tsebea yɛ fofor.",
        "contractors.company_name_label": "Edwumakuw Dzin *",
        "contractors.contact_person_label": "Nyimpa a Yebefrɛ No",
        "contractors.contact_email_label": "Email",
        "contractors.contact_phone_label": "Telefon",
        "contractors.induction_date_label": "Induction Da",
        "contractors.induction_expiry_cap_label": "Induction Ber Twam",
        "contractors.insurance_expiry_cap_label": "Insurance Ber Twam",
        "contractors.competencies_label": "Nyimdzee / Nkrataa (Certifications)",
        "contractors.competencies_placeholder": "dɛ Confined space, EWP licence, HV switching",
        "contractors.notes_label": "Nsɛm a Wɔakyerɛw",
        "contractors.add_btn": "➕ Fa Kɔntrakta Ka Ho",
        "contractors.add_success": "Wɔdze kɔntrakta '{name}' aka ho.",
        "contractors.add_failed": "Kɔntrakta no antum anka ho.",
        "contractors.err_name_required": "Edwumakuw Dzin ho hia.",
        "nav.Owner Console": "Owura Nhwɛsobea",
        "common.save": "Kora Do",
        "common.cancel": "Gyaa",
        "common.submit": "Fa Kɔ",
        "common.delete": "Pepa",
        ...
        "task.btn_upload_attachment": "Fa Krataa Kɔ",
        "task.btn_upload": "Fa Kɔ",
        "task.btn_export_csv": "📥 Yi Ndwuma dɛ CSV",
        "task.caption_full_breakdowns": "Nkyekyɛmu a odzi mu, Pareto nhwehwɛmu, na ka ho amanebɔ wɔ **Analytics** fa mu.",
        "task.caption_no_attachments": "Nkrataa biara nnyi hɔ.",
        "task.caption_no_broadcasts": "Amanebɔ biara nnyi hɔ.",
        "task.caption_no_comments": "Nsɛm biara nnyi hɔ.",
        "task.chk_recurring": "Edwuma a Ɔba Daa (Preventive Maintenance)",
        "task.chk_requires_jsa": "Ohia JSA",
        "task.chk_requires_loto": "Ohia LOTO",
        "task.chk_jsa_signed": "📋 Wɔdze Nsa Ahyɛ JSA Ase",
        "task.share_link_label": "Kyɛ Link",
        "task.chk_loto_isolated": "🔒 Wɔatwa LOTO Mu",
        "task.err_cannot_move_progress": "Wontum mfa nkɔ 'In Progress' a kwan krataa a wɔagye atom nnyi hɔ.",
        "task.err_delete_failed": "Pepa antum anyɛ edwuma. Sɛ ɔkɔ do a, ebia Row Level Security resi nkyerɛw kwan wɔ tasks table no do.",
        "task.err_create_failed": "Edwuma no antum amma.",
        "task.err_comment_failed": "Nsɛm no antum ankɔ.",
        "task.err_title_location_required": "Dzin na Bea ho hia.",
        "task.err_upload_failed": "Antum amfa ankɔ.",
        "task.err_safety_forms_required": "🔒 Ahobammɔ isolation nkrataa ho hia ansaana ebekɔ do.",
        "task.err_permit_required": "🚫 **Edwuma yi hia Edwuma Kwan Krataa a wɔagye atom.** Kwan krataa biara nnyi hɔ a wɔakyerɛw ato do. Ka kyerɛ wo nhwɛsofo na ɔmma bi, na gye tom wɔ Kwan Nkrataa fa mu ansaana efi edwuma ase.",
        "task.info_no_active_users": "Hɔn a wodzi dwuma biara nnyi hɔ.",
        "task.info_no_data": "Nsɛm biara nnyi hɔ a yebekyerɛ.",
        "task.info_no_messages": "Wɔansoma nkra biara.",
        "task.info_no_tasks_assigned": "Wɔamfa edwuma biara amma wo.",
        "task.info_no_tasks_found": "Yeanhu edwuma biara.",
        "task.info_no_tasks_manage": "Edwuma biara nnyi hɔ a ebehwɛ do.",
        "task.info_readonly_directory": "Yi yɛ nkrataa a wubetum akenkan nkoara. Kwan a wɔma, dzibea nsakyeraa, na nsiyaa dze, akawnt wura na ɔhwɛ do.",
        "task.info_owner_note": "Wonye owura — kwan a wɔma na dzibea nsakyeraa wɔ **Owner Console → Access Requests** mu.",
        "task.info_latest_broadcasts": "📢 Amanebɔ a Otwa To:",
        "task.hdr_all_broadcasts": "Amanebɔ Nkra Nyinara",
        "task.hdr_all_tasks": "Nsiesie Ndwuma Nyinara",
        "task.hdr_dispatch_new": "Fa Edwuma Tikiti Fofor Kɔ",
        "task.hdr_full_control": "Edwuma Do Tum Nyinara",
        "task.hdr_recent_broadcasts": "Amanebɔ a Ɔbaa Nnansa Yi",
        "task.hdr_user_directory": "👥 Hɔn a Wodzi Dwuma",
        "task.hdr_task_analytics": "📊 Edwuma Nhwehwɛmu",
        "task.hdr_active_users": "Hɔn a wodzi dwuma",
        "task.hdr_suspended": "Wɔasi hɔn kwan",
        "task.hdr_kpis": "🎯 Edwuma Su Nsusuwii (KPIs)",
        "task.txt_closeout_details": "**Edwuma ewie nsɛm** — yeinom na ɔma nokwardzi na ka ho amanebɔ.",
        "task.txt_already_uploaded": "**📸 Dza wɔdze akɔ dada:**",
        "task.field_failure_code": "Sɛe nɔma (failure code)",
        "task.field_linked_asset": "Mfidzie a Ɔfa Ho (sɛ ɔwɔ hɔ a)",
        "task.field_priority": "Dza Odzi Kan",
        "task.field_recurrence": "Ɔba Daa Ekwan",
        "task.field_update_status": "Sesa Tsebea",
        "task.field_work_type": "Edwuma Ekwan",
        "task.success_attachment": "Wɔdze krataa no akɔ!",
        "task.success_photo": "Wɔdze mfonyin no akɔ!",
        "task.success_safety_checks": "✅ Ahobammɔ nhwehwɛ no adzi nkonyim.",
        "task.success_no_unassigned": "🎉 Edwuma biara nnyi hɔ a wɔamfa amma obiara afei.",
        "task.field_add_comment": "Fa nsɛm ka ho",
        "task.field_location": "Bea / Mpɔtam *",
        "task.field_task_title": "Edwuma Dzin *",
        "task.warn_plotly": "Plotly anaa pandas nnyi hɔ. Yɛsrɛ wo yɛ: pip install plotly pandas",
    },
}
...
# =====================================================================
# AUTO-TRANSLATION — the scalable alternative to hand-maintaining a
# key in 8 language dicts for every one of the ~735 UI strings in this
# app. auto_t("Some English text") returns that text translated into
# the current language, with no dictionary entry required anywhere.
#
# Three cache tiers, cheapest first:
#   1. TRANSLATIONS  — the hand-written dicts above. Checked FIRST on
#      purpose: a human-reviewed translation always beats a machine
#      one, so the existing curated strings (and any corrections a
#      native speaker makes later) are never overridden by AI output.
#   2. Session memory — same string, same run, zero cost.
#   3. Database       — translated once per deployment, ever. Shared
#      across all users, survives restarts.
# Only a genuine miss on all three reaches the AI.
#
# Fails SAFE: any failure (no AI configured, API down, DB error)
# returns the original English rather than blank text or an error. A
# translation problem must never be the thing that stops a supervisor
# reading a permit warning.
# =====================================================================
_AUTO_T_MEMORY = {}

# Built once from the curated dicts: English text -> key, so a string
# that already HAS a human translation is found by its text, not just
# by its key. This is what lets auto_t() prefer human work over AI
# without every call site needing to know which strings are curated.
...
def auto_t(text):
    ...
    # Tier 1 — a human-written translation always wins over AI.
    curated_key = _CURATED_BY_TEXT.get(text)
    if curated_key:
        human = TRANSLATIONS.get(lang, {}).get(curated_key)
        if human:
            return human

    cache_key = (lang, text)
    if cache_key in _AUTO_T_MEMORY:
        return _AUTO_T_MEMORY[cache_key]

    source_hash = hashlib.sha256(text.encode("utf-8")).hexdigest()[:32]

    # Tier 2 — the shared database cache.
    if SUPABASE_AVAILABLE:
        try:
            res = (supabase.table("translation_cache").select("translated_text")
                  .eq("source_hash", source_hash).eq("lang_code", lang).limit(1).execute())
            if res.data:
                result = res.data[0]["translated_text"]
                _AUTO_T_MEMORY[cache_key] = result
                return result
        except Exception as e:
            log_error(str(e), endpoint="auto_t_cache_read")

    # Tier 3 — actually translate. Only reached once per string per
    # language for the entire deployment's lifetime.
    if not AI_FEATURES_AVAILABLE:
        return text
    try:
        lang_name = SUPPORTED_LANGUAGES.get(lang, lang)
        prompt = (
            f"Translate the following UI text from an industrial mine maintenance app "
            f"into {lang_name}. Rules:\n"
            f"- Return ONLY the translation, no explanation, no quotes around it.\n"
            f"- Preserve any emoji, markdown (**bold**), and {{placeholders}} EXACTLY as-is.\n"
            f"- Keep safety acronyms (LOTO, JSA, HV, PPE, DGA) in English, since those are "
            f"what appear on physical equipment tags and permit books.\n"
            f"- Keep it concise — this is a button/label/message in a UI, not prose.\n\n"
            f"Text: {text}"
        )
        translated = generate_smart_text(prompt, max_tokens=300)
        if not translated or not translated.strip():
            return text
        translated = translated.strip()

        # Placeholder integrity check — an AI that drops or renames a
        # {placeholder} would cause a KeyError crash at .format() time
        # downstream. If they don't match exactly, discard the
        # translation and keep English: a page in the wrong language
        # is a minor annoyance, a crashed page is not.
        if set(re.findall(r"\{(\w+)\}", text)) != set(re.findall(r"\{(\w+)\}", translated)):
            log_error(f"Placeholder mismatch, discarded: {text[:60]}", endpoint="auto_t_placeholder")
            return text

        _AUTO_T_MEMORY[cache_key] = translated
        if SUPABASE_AVAILABLE:
            try:
                supabase.table("translation_cache").insert({
                    "source_hash": source_hash, "lang_code": lang,
                    "source_text": text, "translated_text": translated,
                }).execute()
            except Exception as e:
                # A failed cache WRITE is not a failed translation —
                # the text is already correct in memory for this
                # session. Logged, not surfaced.
                log_error(str(e), endpoint="auto_t_cache_write")
        return translated
    except Exception as e:
        log_error(str(e), endpoint="auto_t")
        return text
...
with st.sidebar:
    # Persistent selection tracker — replaces option_menu's own
    # internal state (tied to the old flat "main_nav" widget key,
    # which no longer exists) with an explicit session_state value
    # that survives reruns where no button was just clicked.
    if "_current_nav_section" not in st.session_state:
        st.session_state["_current_nav_section"] = "Task Dashboard"

    # _nav_jump_to (set by navigate_to(), used by Quick Actions and
    # Search) still works exactly as before — it's just consumed
    # directly here now instead of being handed to option_menu's
    # manual_select parameter.
    if st.session_state.get("_nav_jump_to") in nav_options:
        st.session_state["_current_nav_section"] = st.session_state["_nav_jump_to"]
    ...
    for _cat_name, _cat_sections in NAV_CATEGORIES.items():
        ...
        # nothing visible for this user doesn't render an empty
        # dropdown, it just doesn't render at all.
        _visible_items = [s for s in _cat_sections if s in nav_options]
...
# of caveat as the floating button's drag script: relies on an inline
# <script> tag, which this app has documented, established
# unreliability with in Streamlit. Additionally fragile in a second
# way here specifically — Streamlit has no official API for this at
# all (confirmed via streamlit/streamlit#4483, a still-open feature
# request), so this depends on an internal DOM selector
# (data-testid="stSidebarCollapseButton") that already changed once
...
# Global — rendered once here, before the section chain, so it's
# visible on every page regardless of selected_section. Rather than
# duplicate the Chat room's conversation UI in a second, separate
# panel (the prior version of this), this now just navigates directly
# to Chat's Assistant room in a focused, distraction-free mode (no
# room switcher, no other Chat chrome) — one implementation of the
# conversation UI, not two that could drift out of sync with each other.
if AI_FEATURES_AVAILABLE and selected_section != "Chat":
    # Hidden specifically on the Chat section — st.chat_input() also
    # renders as a fixed element pinned to the bottom of the screen
    # (confirmed Streamlit behavior, not assumed), and this button's
    # own fixed position was sitting on top of it, blocking the input
    # bar from view/use. No functional loss: the button's only job is
    ...
    # Draggable positioning — best-effort, not guaranteed. This app has
    # an established, documented history of inline <script> tags (via
    # st.markdown/unsafe_allow_html) not reliably executing in
    # Streamlit — the earlier scroll-to-top fix hit exactly this and
    # had to move to components.html()'s iframe instead. That option
    # isn't available here: components.html() runs inside an iframe
    # and can't reach a real st.button() living in the main document,
    # which is exactly what needs to be dragged. This is the only
    # technically possible path, attempted with defensive, idempotent
    ...
    (function() {
        function initDrag() {
            const btn = document.querySelector('[class*="st-key-floating-ai-toggle"] button');
            if (!btn) { setTimeout(initDrag, 300); return; }
            if (btn.dataset.dragEnabled === "true") return;  // already attached this session
            btn.dataset.dragEnabled = "true";

            // Restore a previously-dragged position, if one was saved —
            // otherwise the CSS default (bottom-center) applies.
            const saved = localStorage.getItem("mwdts_ai_btn_pos");
            if (saved) {
                try {
            ...
            function moveDrag(clientX, clientY) {
                ...
                // Keep it fully on-screen regardless of where it's dropped.
                newLeft = Math.max(4, Math.min(window.innerWidth - btn.offsetWidth - 4, newLeft));
                newTop = Math.max(4, Math.min(window.innerHeight - btn.offsetHeight - 4, newTop));
            ...
            function endDrag() {
                ...
                dragging = false;
                const rect = btn.getBoundingClientRect();
                localStorage.setItem("mwdts_ai_btn_pos", JSON.stringify({left: rect.left, top: rect.top}));
            }

            btn.addEventListener("mousedown", (e) => { startDrag(e.clientX, e.clientY); e.preventDefault(); });
...
# ---- ASSET REGISTER ----
    ...
    if prod_sub == "Recent Shifts":
        ...
        if not _summary_records:
            ...
            render_subheading("Total by material", level=4)
...
# ---- INVENTORY ----
    ...
    render_section_header("📦 Inventory & Parts Management")
...
# ---- INCIDENT REPORTS ----
    ...
    render_section_header(t("incidents.title"))
    can_manage_incidents = can(role, "incident.investigate")

    # Sub-nav uses translated labels for DISPLAY, but routes on the
    # canonical English value — same reasoning as the main sidebar
    # nav's translation handling. Translating "Report Incident" itself
...
# ---- CHAT ROOM ----
    ...
    render_section_header(t("permits.title"))
    ...
    if _wr_tab == "Submit My Report":
        ...
        if _comprehensive["worker_count"] == 0:
            ...
            _cc1.metric("Workers Reported", _comprehensive["worker_count"])
            _cc2.metric("Total Completed", _comprehensive["total_completed"])
            _cc3.metric("Total Incomplete", _comprehensive["total_incomplete"])

            if _comprehensive["combined_items_used"]:
                render_subheading("Combined Items Used", level=5)
                for item in _comprehensive["combined_items_used"]:
                    st.write(f"- {esc(item['part_name'])}: {item['quantity']:g}")

            if _comprehensive["all_locations"]:
                st.caption("📍 Locations covered: " + ", ".join(esc(l) for l in _comprehensive["all_locations"]))
    ...
    render_section_header(t("contractors.title"))
    ...
    if require(role, "analytics.view"):
        render_section_header("📈 Maintenance & Safety Analytics")
        tasks = st.session_state.tasks
    ...
    _focused_mode = st.session_state.get("_focused_assistant_mode", False)

    if _focused_mode:
        # A clean, distraction-free view — no room switcher, no other
        # Chat chrome, just the conversation. This is specifically
        ...
        <div style="text-align:center; padding: 0.5rem 0 1rem 0;">
            <i class="fas fa-robot" style="font-size: 2.4rem; color: var(--brand-lime);"></i>
        ...
        st.markdown("""
        <div style="text-align:center; padding: 0.3rem 0 1rem 0;">
            <i class="fas fa-comments" style="font-size: 2rem; color: var(--brand-lime);"></i>
    ...
    if room == "global":
        ...
        # single-option selector would be UI noise with nothing to
        # decide. Options come from get_configured_ai_providers(), so
        # this can never offer a provider that has no key set.
        _configured_providers = get_configured_ai_providers()
        ...
        # list, which was only isolated per BROWSER SESSION, not per
        # user account. On a shared device (a workstation or tablet
        # multiple workers log into across a shift), the old version
        # could carry one person's chat straight into the next
        # person's session if they logged in on the same tab without
        # a full page reload. Every read/write here is scoped to
        # `username` specifically to close that gap, not just move it.
        _ai_history = fetch_ai_chat_history(username)
        ...
        if not _ai_history:
            ...
            for _msg in _ai_history:
                with st.chat_message("user" if _msg["role"] == "user" else "assistant",
                                    avatar="🧑" if _msg["role"] == "user" else "🤖"):
                    st.markdown(_msg["content"])

        # Web search — a genuinely separate, explicit opt-in. Only
        # shown when Groq is configured, since this is specifically
        # Groq's Compound Mini capability, not something every
        # provider offers. Cost stated plainly in the label itself
        # rather than buried in a tooltip, since this is real money
        # ($5-8 per 1,000 searches, per Groq's own pricing) on top of
        # what's otherwise a free-tier feature everywhere else in
        # this app — a person should see that before turning it on,
        # not discover it later on a bill.
        ...
        if _ai_question:
            with st.chat_message("user", avatar="🧑"):
                st.markdown(_ai_question)
            with st.chat_message("assistant", avatar="🤖"):
                with st.spinner("Searching the web..." if _web_search_on else "Thinking..."):
                    if _web_search_on:
                        # A genuinely different path, not routed through
                        # generate_smart_text()'s fall-through chain —
                        # this must NEVER silently fall back to a
                        # different (free) provider on failure, since
                        # that would misrepresent that a web search
                        # actually happened when it didn't.
                        try:
                            _answer, _web_sources = call_groq_web_search(_ai_question)
                            _actual_provider = "Groq (web search)"
                        except Exception as e:
                            log_error(str(e), endpoint="call_groq_web_search")
                            _answer, _web_sources, _actual_provider = None, [], None
                    else:
                        _web_sources = []
                        ...
                        # History passed to the AI is this same per-user
                        # DB history — genuine multi-turn memory that
                        # survives a page reload or new device, not just
                        # within-session recall.
                        _preferred_provider = st.session_state.get("_ai_preferred_provider", "Auto (default order)")
                        if _preferred_provider == "Auto (default order)":
                            _preferred_provider = None
                        _answer, _actual_provider = ask_maintenance_assistant(
                            _ai_question, _context, _ai_history, preferred_provider=_preferred_provider)
                if _answer:
                    st.markdown(_answer)
                    if _web_sources:
                    ...
                    # this is the fix for "switching providers doesn't
                    # work": that selection was always being respected
                    # when possible, but a failed one fell back
                    # silently, with nothing distinguishing it from a
                    # genuine success on the chosen provider. Now a
            ...
            if _answer:
                ...
                # question with no answer if something failed partway.
                save_ai_chat_message(username, "user", _ai_question)
                save_ai_chat_message(username, "assistant", _answer)
                st.rerun()
...
# ---- FEEDBACK / SUGGESTIONS BOARD ----
    ...
    render_section_header("💡 App Feedback & Suggestions")
...
# ---- PROFILE TAB ----
    ...
    render_section_header("👤 User Profile")
    st.markdown(render_meta_chips([
...
# ---- ACTIVITY TIMELINE ----
    ...
    render_section_header("ℹ️ About MWDTS")
...
""")
    ...
    if not _cals:
        ...
        render_subheading("All Tracked Instruments", level=5)
        for c in _cals:
            _next_due, _days_until, _status = instrument_calibration_status(c)
    ...
    if _transformer_tags:
        render_subheading("Fleet Summary", level=4)
        _fleet_rows = []
    ...
    if not _relay_tags:
        ...
        if _as_found_records and _as_left_records:
            render_subheading("Latest As-Found vs As-Left Comparison", level=5)
    ...
    if not _af_studies:
        ...
        render_subheading("All Tracked Panels", level=4)
