...
def render_section_header(title_with_emoji, subtitle=None):
    ...
    title_text = parts[1] if len(parts) > 1 else title_with_emoji
    sub_html = f'<p style="color: var(--text-secondary); margin: 0;">{esc(subtitle)}</p>' if subtitle else ""
    st.markdown(f"""
...
def generate_smart_text(prompt, max_tokens=300, _return_error=False, preferred_provider=None):
    ...
    return (None, _last_err) if _return_error else None
...
def ask_maintenance_assistant(question, context_summary, conversation_history=None, preferred_provider=None):
    ...
    return generate_smart_text(messages, max_tokens=300, preferred_provider=preferred_provider)
...
# ---- CHAT ROOM ----
    ...
    if _wr_tab == "Submit My Report":
        ...
        st.markdown("##### Draft (auto-generated)")
        ...
        if _comprehensive["worker_count"] == 0:
            ...
            if _comprehensive["combined_items_used"]:
                st.markdown("##### Combined Items Used")
        ...
        if _ai_question:
            ...
            with st.chat_message("assistant", avatar="🤖"):
                with st.spinner("Thinking..."):
                    ...
                    _answer = ask_maintenance_assistant(_ai_question, _context, _ai_history,
