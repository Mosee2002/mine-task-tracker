...
def generate_smart_text(prompt, max_tokens=300, _return_error=False):
    ...
    OpenAI, Gemini, then Groq, in that order. Shared by all
    AI-assisted features (Smart Work Order Descriptions, Incident
    Severity Prediction, AI Assistant) rather than each
    ...
    ...} dicts for genuine multi-turn conversation. All four providers
    accept a messages/contents array in a compatible shape (Gemini's
    is converted below; the other three are used as-is).
    ...
    if ANTHROPIC_API_KEY:
        try:
            ...
            log_error(_last_err, endpoint="generate_smart_text_anthropic")
    ...
    if OPENAI_API_KEY:
        try:
            ...
            _last_err = f"{type(e).__name__}: {e}"
            log_error(_last_err, endpoint="generate_smart_text_openai")
    ...
    if GEMINI_API_KEY:
        try:
            ...
            # Redacted here, once, before this string goes anywhere else
            # (logged to app_errors, or returned to the UI via
            # _return_error) — a failed call must never be the thing
            # that leaks a live key into a log a supervisor can read.
            _last_err = re.sub(r"key=[^&\s]+", "key=***REDACTED***", f"{type(e).__name__}: {e}")
            log_error(_last_err, endpoint="generate_smart_text_gemini")

    if GROQ_API_KEY:
        try:
            # Genuinely OpenAI-compatible, not just similar — same
            # request/response shape as the OpenAI branch above,
            # verified directly against Groq's own docs and GitHub
            # repo before writing this, not assumed from the
            # marketing claim alone. Only the base URL, key, and
            ...
            _last_err = f"{type(e).__name__}: {e}"
            log_error(_last_err, endpoint="generate_smart_text_groq")

    if OPENROUTER_API_KEY:
        try:
            # Also genuinely OpenAI-compatible (verified directly
            # against OpenRouter's own docs before writing this) —
            # the one real difference from the Groq/OpenAI branches
            # is the model string needs a provider prefix
            # (OPENROUTER_MODEL already carries this, e.g.
            # "openai/gpt-4o-mini"), not a bare model name.
            resp = requests.post(
            ...
            _last_err = f"{type(e).__name__}: {e}"
            log_error(_last_err, endpoint="generate_smart_text_openrouter")
