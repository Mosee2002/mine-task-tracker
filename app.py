...
/* The header is now position:fixed, which removes it from normal
   ...
   rather than below it. 175px = 60px for Streamlit's own native
   header (which this app's header now sits below, not on top of —
   see the comment above main-header) + 115px for this app's own
   header (padding + title line + subtitle line, measured from its
   own CSS values, with headroom for text wrapping to two lines on
   narrow screens). */
.main-header-spacer { height: 175px; }
...
if not st.session_state.authenticated and not st.session_state["_pre_login_welcome_dismissed"]:
    ...
    with _rcol1:
        if st.button("👷 Worker", use_container_width=True, type="primary"):
    ...
    with _rcol2:
        if st.button("🧑‍🔧 Supervisor", use_container_width=True, type="primary"):
            ...
            st.rerun()
    with _rcol3:
        if st.button("🦺 Superintendent", use_container_width=True, type="primary"):
            st.session_state["_pre_login_welcome_dismissed"] = True
            st.rerun()

    # Deliberately small and unlabeled-as-"admin" in the button text —
    # this isn't a security boundary (the real check happens at login,
    # same as every other role), just not something to visually
    # advertise alongside the three above.
