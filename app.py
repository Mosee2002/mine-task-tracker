...
# Base URL for generating shareable deep links (Deep Linking feature) —
# Streamlit doesn't expose the app's own current URL to Python code
# (no reliable way to read window.location.origin without a round
# trip through JavaScript), so this is configured explicitly instead,
# same reasoning as MINE_LATITUDE/LONGITUDE above. Defaults to this
# app's known current deployment URL so links work out of the box,
# but should be overridden in secrets.toml if the app is ever
# redeployed to a different URL.
APP_BASE_URL = _secret_get("APP_BASE_URL", "https://mine-task-tracker-ljc2ppwmwiwtfqwykn2by5.streamlit.app")
            ...
            if not my_tasks:
                ...
                for idx, task in enumerate(my_tasks):
                    ...
                    with st.expander(f"🔗 {t('task.share_link_label')}"):
                        st.code(f"{APP_BASE_URL}/?task={task['id']}", language=None)
    ...
    if inc_sub in ("All Incidents", "My Reports"):
        ...
        with st.form("new_incident_form", clear_on_submit=True):
            ...
            with _rc1:
                ...
                severity = st.selectbox(t("incidents.severity_label"), ["Low", "Medium", "High", "Critical"])
...
# ---- CHAT ROOM ----
    ...
    if require(role, "analytics.view"):
        ...
        analytics_sub = option_menu(
            menu_title=None,
            options=["Overview", "Reliability", "Utilization", "Backlog & Compliance", "Failure Pareto", "Cost", "Safety"],
