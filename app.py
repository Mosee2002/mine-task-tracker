...
[class*="st-key-floating-ai-panel"] {
    position: fixed !important;
    bottom: 90px;
    right: 20px;
    left: 20px;
...
def generate_smart_text(prompt, max_tokens=300, _return_error=False):
    ...
    Severity Prediction, Maintenance Assistant) rather than each
...
# visible on every page regardless of selected_section. Shares
# _ai_assistant_history with the Chat -> Assistant room (built
# earlier) rather than keeping a separate conversation, so asking a
# question here and continuing it in Chat later sees the same
# history either way. Kept as its own compact implementation instead
# of refactoring the Chat room's logic to be shared — that logic is
# already working and tested; duplicating a small amount of it here
# is a smaller risk than restructuring it.
if AI_FEATURES_AVAILABLE:
    ...
    with st.container(key="floating-ai-toggle"):
        if st.button("💬" if not st.session_state["_floating_ai_open"] else "✕", key="floating_ai_toggle_btn"):
            st.session_state["_floating_ai_open"] = not st.session_state["_floating_ai_open"]
            st.rerun()

    if st.session_state["_floating_ai_open"]:
        with st.container(key="floating-ai-panel"):
            st.markdown("**🤖 AI Assistant** — app questions or general knowledge")
            for _fa_msg in st.session_state["_ai_assistant_history"][-6:]:
                # Last 6 only (3 exchanges) — this is a quick-access
                # panel, not the full Chat room view; the complete
                # history is still there if opened from Chat instead.
                if _fa_msg["role"] == "user":
                    _fa_display = _fa_msg["content"].split("Question: ", 1)[-1]
                    st.markdown(f"**You:** {esc(_fa_display)}")
                else:
                    st.markdown(f"**Assistant:** {esc(_fa_msg['content'])}")
            _fa_question = st.text_input("Ask anything...", key="floating_ai_input", label_visibility="collapsed")
            if st.button("Send", key="floating_ai_send"):
                if _fa_question.strip():
                    with st.spinner("Thinking..."):
                        _fa_parts_lookup = {p['id']: p for p in st.session_state.get("parts", [])}
                        _fa_context = get_app_context_summary(st.session_state.tasks, st.session_state.get("assets", []),
                                                              st.session_state.incidents, _fa_parts_lookup)
                        _fa_answer = ask_maintenance_assistant(
                            _fa_question, _fa_context, st.session_state["_ai_assistant_history"])
                    if _fa_answer:
                        st.session_state["_ai_assistant_history"].append(
                            {"role": "user", "content": f"Question: {_fa_question}"})
                        st.session_state["_ai_assistant_history"].append(
                            {"role": "assistant", "content": _fa_answer})
                        st.rerun()
                    else:
                        st.error("Couldn't get an answer right now.")
